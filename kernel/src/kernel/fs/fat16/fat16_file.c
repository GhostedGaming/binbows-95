...
void fat16_write_clusters(uint8_t drive, uint16_t first_cluster, const uint8_t *data, uint32_t size) {
    ...
    if (ide_read_sectors(drive, bpb.fat_size_16, bpb.reserved_sector_count, fat) != 0) {
        serial_printf("Failed to read FAT in fat16_write_clusters\n");
        return;
    }
    ...
    while (remaining > 0 && current_cluster >= 2 && current_cluster < FAT16_EOC) {
        ...
        if (remaining > 0) {
            uint16_t next_cluster = fat16[current_cluster];
...
int fat16_write_file(uint8_t drive, const char* filename, const uint8_t* data, uint32_t size) {
    ...
    uint16_t first_cluster = fat16_alloc_clusters(drive, clusters_needed);
    if (first_cluster == 0) {
        serial_printf("No free clusters available\n");
        return -1;
    }

    int dir_entry = fat16_find_free_root_dir_entry(drive, &bpb);
    if (dir_entry == -1) {
        serial_printf("No free directory entry\n");
        return -1;
    }

    fat16_write_dir_entry(drive, &bpb, dir_entry, filename, first_cluster, size);
    fat16_write_clusters(drive, first_cluster, data, size);
    ...
    for (uint32_t i = 0; i < root_dir_sectors; i++) {
        ...
        for (uint32_t j = 0; j < 512; j += 32) {
            ...
            if ((entry->attr & ATTR_DIRECTORY) || (entry->attr & ATTR_VOLUME_ID)) {
                continue; // Skip directories and volume labels
            }
            ...
            if (memcmp(entry->name, fat_filename, 11) == 0) {
                ...
                while (cluster >= 2 && cluster < FAT16_EOC && bytes_read < file_size) {
