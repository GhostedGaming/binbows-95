...
void zero_drive(HBA_PORT *port) {
    ...
    uint32_t tmp32;
    memcpy(&tmp32, &identify_buf[60], sizeof(tmp32));
    total_sectors = tmp32;

    if (identify_buf[83] & (1 << 10)) {
        uint64_t tmp64;
        memcpy(&tmp64, &identify_buf[100], sizeof(tmp64));
        total_sectors = tmp64;
    }

    serial_printf("Zeroing %llu sectors...\n", total_sectors);

    static uint8_t zero_buf[512] = {0};

    uint64_t written = 0;
    while (written < total_sectors) {
        uint32_t batch = 1;
        if (written + batch > total_sectors) batch = total_sectors - written;

        if (!write_sectors(port, (uint32_t)written, (uint32_t)(written >> 32), batch, zero_buf)) {
            serial_printf("Failed at sector %llu\n", written);
            break;
        }

        written += batch;
        if (written % 1000 == 0) {
            serial_printf("Zeroed %llu / %llu sectors\n", written, total_sectors);
        }
    }

    serial_printf("Drive zeroed\n");
}
